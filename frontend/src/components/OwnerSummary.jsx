  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-baseline">
        <h3 className="font-extrabold text-base font-kanit text-neutral-800">ข้อมูลเชิงลึก &amp; ส่งออก</h3>
      ...
      <div>
        <label className="block text-[10px] font-bold text-neutral-500 mb-1">เลือกเดือน &amp; ปี</label>
        <select
          value={month}
          onChange={e => setMonth(e.target.value)}
          className="w-full border rounded-xl p-2.5 bg-white text-xs font-bold focus:outline-none focus:ring-1 focus:ring-amber-500"
        >
          {months.map(m => (
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-white border rounded-2xl p-2.5 shadow-xs">
          <span className="text-[9px] font-extrabold text-neutral-400 font-kanit block">รายรับ</span>
          <span className="font-mono text-sm font-extrabold text-green-700">{baht(totals.income)}</span>
        </div>
        <div className="bg-white border rounded-2xl p-2.5 shadow-xs">
          <span className="text-[9px] font-extrabold text-neutral-400 font-kanit block">รายจ่าย</span>
          <span className="font-mono text-sm font-extrabold text-red-600">{baht(totals.expense)}</span>
        </div>
        <div className="bg-white border rounded-2xl p-2.5 shadow-xs">
          <span className="text-[9px] font-extrabold text-neutral-400 font-kanit block">กำไรสุทธิ</span>
      ...
      <button
        ...
        className="w-full bg-[#4A2C17] hover:bg-[#3A2212] text-white font-bold py-3 rounded-2xl transition flex items-center justify-center gap-2 font-kanit"
      >
        ...
        {rows.length > 0 ? (
          rows.map(r => (
            ...
            >
              ...
              <span className={r.kind === 'income' ? 'text-green-700 font-bold' : 'text-red-600 font-bold'}>
